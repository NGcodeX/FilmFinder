import React from 'react';

import { Typography, Grid, Grow, Tooltip, Rating } from '@mui/material';
import { Link } from 'react-router-dom';

import useStyles from './styles';
//TODO:From 600px to 780 the ui is not very user friendly , so we have to add a media query for that and reduce the size of each movie poster
function Movie({ movie, index }) {
  const classes = useStyles();
  // console.log(movie, index);
  return (
    <Grid item xs={12} sm={6} md={4} lg={3} xl={2.4} className={classes.movie}>

      <Grow in key={index} timeout={(index + 1) * 250}>
        <Link className={classes.links} to={`/movie/${movie.id}`}>

          <img
            alt={movie.title}
            className={classes.image}
            src={movie.poster_path ? `https://image.tmdb.org/t/p/w500/${movie.poster_path}` : 'https://www.fillmuray.com/200/300'}
          />

          {/*
                    {movie.poster_path ?
                    <img alt={movie.title} className={classes.image} src={`https://image.tmd.org/t/p/w500/${movie.poster_path}`}/>:
                    <img alt={movie.title} className={classes.image} src='https://www.fillmuray.com/200/300'/>} */}
          <Typography className={classes.title} variant="h5">{movie.title}
          </Typography>
          <Tooltip disableTouchListener title={`${movie.vote_average} / 10 `}>
            <div>
              <Rating readOnly value={movie.vote_average / 2} precision={0.1} />
            </div>
          </Tooltip>
        </Link>
        {/* A transition that material ui uses */}
      </Grow>
    </Grid>
  )
}

export default Movie
